 ...
 */
package javaapplications7_2dva;
 ...
 * @author student
 */
public class JavaApplicationS7_2dva {
    ...
    public static void main(String[] args) throws InvalidAlgorithmParameterException {
        ...
        try {
            ...
            Logger.getLogger(JavaApplicationS7_2dva.class.getName()).log(Level.SEVERE, null, ex);
        } catch (NoSuchPaddingException ex) {
            Logger.getLogger(JavaApplicationS7_2dva.class.getName()).log(Level.SEVERE, null, ex);
        }
        try {
            // initialize AES objecy to encrypt mode
            IvParameterSpec iv_param = new IvParameterSpec(new byte[16]);
            myAES.init(Cipher.ENCRYPT_MODE, myKey, iv_param);
        } catch (InvalidKeyException ex) {
            Logger.getLogger(JavaApplicationS7_2dva.class.getName()).log(Level.SEVERE, null, ex);
        }
        // initialize plaintext
        byte[] plaintext = new byte[20];
        plaintext = "ana are mere 111 222".getBytes();
        ...
        try {
            ...
            IvParameterSpec iv_param = new IvParameterSpec(new byte[16]);
            myAES.init(Cipher.DECRYPT_MODE, myKey, iv_param);
        } catch (InvalidKeyException ex) {
            Logger.getLogger(JavaApplicationS7_2dva.class.getName()).log(Level.SEVERE, null, ex);
        }
        // initialize a new array of bytes to place the decryption
        byte[] dec_plaintext = new byte[20];
        try {
